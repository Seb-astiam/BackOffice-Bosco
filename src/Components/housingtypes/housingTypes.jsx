  ...
  const fetchTypes = async () => {
    try {
      const response = await axios.get(
        `/housingtype/alltypes?page=${currentPage}&limit=${servicesPerPage}`
      );
    ...
    try {
      ...
      if (actionType === "create") {
        ...
        Swal.fire({
          ...
          text: "El tipo ha sido registrado correctamente.",
        });
      } else if (actionType === "update") {
        response = await axios.put(
          `/housingtype/${selectedService.id}`,
          newServiceName
        );
        Swal.fire({
          icon: "success",
    ...
    <div className="overflow-x-auto">
      ...
      >
        <div className="p-4 max-w-md mx-auto">
          ...
          <h2 className="text-lg font-bold mb-4">{actionType === "create" ? "Crear Alojamiento" : "Actualizar Alojamiento"}</h2>
